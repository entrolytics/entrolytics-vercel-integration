...
export async function uninstallInstallation(
  ...
  try {
    ...
    console.log("[Uninstall] Access token deleted for installation:", installationId);
  } catch (error) {
...
export async function provisionResource(
  ...
  if (request.metadata?.projectId) {
    try {
      ...
      await upsertEnvironmentVariables(installationId, request.metadata.projectId as string, [
        {
      ...
      console.error("[Resource] Failed to inject environment variables:", error);
 ...
 * Delete a resource
 */
export async function deleteResource(installationId: string, resourceId: string): Promise<void> {
  ...
  if (resource?.metadata?.projectId) {
    try {
      ...
      await deleteEnvironmentVariables(installationId, resource.metadata.projectId as string, [
      ...
      console.error("[Resource] Failed to remove environment variables:", error);
    }
  }
 ...
 */
export async function listResources(installationId: string): Promise<{ resources: Resource[] }> {
  const resourceIds = await kv.lrange<string>(`${installationId}:resources`, 0, -1);
...
async function createEntrolyticsWebsite(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Failed to create Entrolytics website: ${response.status} ${errorText}`);
