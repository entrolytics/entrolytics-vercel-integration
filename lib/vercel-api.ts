 ...
 */
export async function getAccessToken(installationId: string): Promise<VercelTokenData | null> {
...
export async function upsertEnvironmentVariables(
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to create environment variables: ${response.status}`);
  }
...
export async function deleteEnvironmentVariables(
  ...
  for (const envVar of varsToDelete) {
    const url = new URL(`https://api.vercel.com/v9/projects/${projectId}/env/${envVar.id}`);
    if (tokenData.team_id) {
      url.searchParams.set("teamId", tokenData.team_id);
    }
...
export async function getAccountInfo(installationId: string): Promise<{
  ...
  if (tokenData.team_id) {
    const response = await fetch(`https://api.vercel.com/v2/teams/${tokenData.team_id}`, {
      headers: {
        Authorization: `Bearer ${tokenData.access_token}`,
      },
    });
