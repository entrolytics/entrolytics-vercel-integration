...
export const PUT = withAuth(async (claims, request) => {
  const body = await readRequestBodyWithSchema(request, installIntegrationRequestSchema);
...
export const GET = withAuth(async (claims) => {
  ...
  const billingPlan = billingPlans.plans.find((plan) => plan.id === installation.billingPlanId);

  return NextResponse.json({
    billingPlan: billingPlan ? { ...billingPlan, scope: "installation" } : undefined,
  });
});
