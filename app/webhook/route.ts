...
import { storeWebhookEvent, uninstallInstallation, getInstallationConfig } from '@/lib/partner';
import { webhookEventSchema, unknownWebhookEventSchema, type WebhookEvent } from '@/lib/schemas';
...
async function trackDeployment(payload: {
  ...
  try {
    ...
    if (!config?.websiteId || !config?.apiKey) {
      console.log('[Webhook] No website ID or API key configured for project:', payload.project?.id);
      return;
    }
    ...
    const deploymentPayload = {
      ...
      deployUrl: payload.deployment.url ? `https://${payload.deployment.url}` : undefined,
      source: 'vercel',
    };

    const response = await fetch(`${entrolyticsHost}/api/websites/${config.websiteId}/deployments`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      console.log('[Webhook] Deployment tracked to entrolytics-ng:', payload.deployment.id);
    } else {
      console.error('[Webhook] Failed to track deployment:', await response.text());
    }
  } catch (error) {
