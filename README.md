...
- `NEXT_PUBLIC_ENTROLYTICS_ENDPOINT` - Collection endpoint (defaults to `/collect`)

## Edge Optimization

The integration automatically configures your app to use the canonical collection endpoint (`/collect`) for analytics ingestion. This provides:
...
```bash
NEXT_PUBLIC_ENTROLYTICS_ENDPOINT=/collect      # Direct canonical ingestion (default)

# or

NEXT_PUBLIC_ENTROLYTICS_ENDPOINT=/api/collect  # Local proxy route (if you expose one)
